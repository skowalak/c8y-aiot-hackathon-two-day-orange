#!/usr/bin/env bash
# Package the device simulator as a Cumulocity microservice ZIP.
#
# Cumulocity wants a `docker save` tarball plus the manifest. Building that
# needs no Docker daemon: the simulator is a static Go binary, so the image is one
...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
  go build -trimpath -ldflags="$ldflags" -o "$work/rootfs/simulatord" .
